...
if __name__ == "__main__":
    ...
    parser.add_argument("-if", "--input_file", help="csv file with the train and dev inputs(features & labels)",
                        type=str,
    ...
    x = x.reshape(n_examples, n_features, 1)
    ...
    if args.train is True:
        ...
        layers = [tf.keras.layers.Input(batch_shape=(None, n_features, 1))]

        # Layer 1
        layers = layers + [
            tf.keras.layers.Conv1D(filters=6, kernel_size=5, strides=1, padding='valid', activation='relu',
        ...
        # Layer 3
        layers = layers + [tf.keras.layers.Dense(120, kernel_initializer='glorot_normal',
                                                 activation='relu')]

        # Layer 4
        layers = layers + [tf.keras.layers.Dense(84, kernel_initializer='glorot_normal',
                                                 activation='relu')]
    ...
    x_test = x_test.reshape(n_test_examples, n_features, 1)
    y_test = y_test.reshape(n_test_examples, 1, n_outputs)

    # compute the metrics for training, dev and test set
    log.info(f"Start generating the metrics for Convolutional NN")
    ...
    log.info(f"Ended generating the metrics for Convolutional NN")
